...
/// Placeholder for attachments whose filename metadata is missing.
pub(crate) const ATTACHMENT_NO_FILENAME: &str = "Attachment missing name metadata!";

/// The character to replace disallowed chars with in generated filenames.
...
/// Remove unsafe chars in generated chat filenames. Mirrors the helper from
/// the upstream `imessage-exporter` `app::sanitizers` module.
...
impl Config {
    ...
    // MARK: Startup
    /// Validate options, prepare output directories, and run the JSON export.
    pub fn start(&self) -> Result<(), RuntimeError> {
        if let Some(export_type) = &self.options.export_type {
            ...
            // Only JSON is supported in this crate; the match keeps export_type live for
            // future variants without changing call sites.
            match export_type {
                ExportType::Json => {
...
mod filename_tests {
    use crate::{
        Config, Options,
        {contacts::Name, runtime::MAX_LENGTH},
    };
    ...
    #[test]
    fn can_create() {
        let mut options = Options::fake_options(crate::options::ExportType::Json);
        // Disable the export
    ...
    fn can_get_filename_good() {
        let options = Options::fake_options(crate::options::ExportType::Json);
    ...
    fn can_get_filename_chat_display_name_long() {
        ...
        assert_eq!(
            filename,
            "Life is infinitely stranger than anything which the mind of man could invent. We would not dare to conceive the things which are really mere commonplaces of existence. If we could fly out of that window hand in hand, hover over th - 0.json"
        );
    }
    ...
    fn can_get_filename_chat_display_name_normal() {
        ...
        // Get filename
        let filename = app.filename(&chat);
        assert_eq!(filename, "Test Chat Name - 0.json");
    }
    ...
    fn can_get_filename_chat_participants() {
        ...
        // Get filename
        let filename = app.filename(&chat);
        assert_eq!(filename, "Person 10, Person 11.json");
    }
    ...
    fn can_get_filename_chat_no_participants() {
        ...
        // Get filename
        let filename = app.filename(&chat);
        assert_eq!(filename, "Default.json");
    }
    ...
    fn can_get_filename_chat_display_name_truncated_emoji() {
        ...
        let filename = app.filename(&chat);
        assert!(filename.len() <= MAX_LENGTH + 20); // suffix " - 0.json" adds some
...
mod who_tests {
    use crate::{
        Config, Options,
        {contacts::Name, runtime::filename_tests::fake_chat},
    };
...
mod chat_filter_tests {
    ...
    use crate::{
        Config, Options,
        {contacts::Name, options::ExportType},
    };

    #[test]
    fn can_generate_filter_string_multiple() {
        let mut options = Options::fake_options(ExportType::Json);
